...
//
//    vec2 Where_pos = vec2 (0,0);
//    // ctf5 red flag 1041, 1969
//    // ctf5 blue flag 7566, 1969
//    // ctf5 center 4304, 2257
//
//    if(pResult->NumArguments()>0) {
//        pSelf->Console()->Print(1, "World", "Found args...", true);
//        float x = pResult->GetFloat(1);
//        float y = pResult->GetFloat(2);
//        Where_pos+=vec2 (x,y);
//        for (int i = 1; i <= pSelf->m_MP.length(); i++){
//            if(pSelf->)
//        }
//        for (int i = 0; i < MAX_CLIENTS; i++) {
//            if (pSelf->m_apPlayers[i]) {
//                if(Who == -1){
//                    if (str_comp("me", pResult->GetString(0)) == 0){
//                        if (str_comp(pSelf->Server()->ClientName(i), "Silent") == 0) {
//                            Who=i;
//                        }
//                    } else{
...
//                }
//                if (Who!= -1){
//                    pSelf->Console()->Print(1, "World", "Foud matching player, teleporting...", true);
//                    pSelf->m_apPlayers[Who]->GetCharacter()->Teleport(Where_pos);
//                }
//            }
//        }
...
void CGameContext::ConVoteAutomode(IConsole::IResult *pResult, void *pUserData){
    CGameContext *pSelf = (CGameContext *)pUserData;
...
void CGameContext::ConVoteNinja(IConsole::IResult *pResult, void *pUserData){
    CGameContext *pSelf = (CGameContext *)pUserData;
...
void CGameContext::ConKeepCheat(IConsole::IResult *pResult, void *pUserData){
    CGameContext *pSelf = (CGameContext *)pUserData;
    std::string username;
    if(pResult->NumArguments()>0) {
...
void CGameContext::OnInitMap(int MapID)
{
    ...
    char aBuf[128];
    str_format(aBuf, sizeof(aBuf), "Initialized new Map with ID '%d'", MapID);
